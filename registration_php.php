                ...
                if (move_uploaded_file($abstract_file,$absract_file_destination)) {
                    ...
                    if ($details_sql) {
                        ...
                        if ($user_sql=mysqli_query($connect,$user)) {
                            ...
                            $message = " Hi ".$first_name." ".$last_name.",Your journal registered successfully, your journal Id is ".$journal_picture_name.",your journal will publish after verification.";
                            $headers = "From:" . $from;
                            if (mail($to1, $subject1,$message,$headers)){
                                    echo "<script>alert('Request submitted successfully')</script>";
                                    echo "<script>window.location='registration.php';</script>";
                            }else{
                                    echo "<script>alert(' response mail cannot be sent rejected by server,Contact WISDOMWRITE ')</script>";
                                    echo "<script>window.location='registration.php';</script>";
                            }
                        //--------------------- /Mail -------------------
